...
#define SIMD_LOAD_HALF(addr)                                                   \
    _mm512_cvtph_ps(_mm256_castps_si256(_mm256_load_ps((addr))))

#define SIMD_STORE_HALF(addr, what)                                            \
    _mm256_store_ps(addr, _mm256_castsi256_ps(_mm512_cvtps_ph(what, 0)))
...
#pragma unroll(RBW)
            ...
            {
                vout[rbd][rbh][rbw] = SIMD_LOAD_HALF(
                    o + rbw * SIMD_WIDTH / 2 + rbd * SIMD_WIDTH / 2 * 100 +
                    rbh * SIMD_WIDTH / 2 * 10);
            }
    ...
    for (int f = 0; f < FIN; ++f)
        for (int kd = 0; kd < 1; ++kd)
            for (int kh = 0; kh < 3; ++kh)
                ...
                {
                    vwt =
                        SIMD_SET1(k[kh * 3 + kw + kd * 3 * 3 + f * 3 * 3 * 1]);
...
#pragma unroll(RBW)
                            for (int rbw = 0; rbw < RBW; ++rbw)
                            {
                                vout[rbd][rbh][rbw] = SIMD_FMADD(
                                    ...
                                    SIMD_LOAD_HALF(
                                        i +
                                        (kd + rbd) * SIMD_WIDTH / 2 * 12 * 12 +
                                        (kh + rbh) * SIMD_WIDTH / 2 * 12 +
                                        (kw + rbw) * SIMD_WIDTH / 2 +
                                        f * 12 * 12 * 1 * SIMD_WIDTH / 2),
                                    vout[rbd][rbh][rbw]);
                            }
                }
...
#pragma unroll(RBW)
            for (int rbw = 0; rbw < RBW; ++rbw)
            {
                SIMD_STORE_HALF(o + rbw * SIMD_WIDTH / 2 +
                                    rbd * SIMD_WIDTH / 2 * 100 +
                                    rbh * SIMD_WIDTH / 2 * 10,
                                vout[rbd][rbh][rbw]);
            }
}
...
int main()
{
    std::vector<std::function<void()>> tasks(64 * 2);
    ...
    for (int i = 0; i < 64; ++i)
    {
        tasks[i] = []() {
            ...
            float* a = (float*)Amalloc(sizeof(float) * 2264 * 64 *
                                       64); // new float[64*64*64];
            float* b = (float*)Amalloc(sizeof(float) * 2264 * 64 *
                                       64); // new float[64*64*64];
            float* c = (float*)Amalloc(sizeof(float) * 2264 * 64 *
                                       64); //  new float[64*64*64];
...
#define RBZ 10
            ...
            double gflops = static_cast<double>(SIMD_WIDTH * 2 * 2 * RBZ * 3 *
                                                3 * SIMD_WIDTH) /
                            1000000000;
