...
#pragma unroll(RBW)
            ...
            {
                vout[rbd][rbh][rbw] =
                    SIMD_LOAD(o + rbw * SIMD_WIDTH + rbd * SIMD_WIDTH * 100 +
                              rbh * SIMD_WIDTH * 10);
            }
...
#pragma unroll(RBW)
                            for (int rbw = 0; rbw < RBW; ++rbw)
                            {
                                vout[rbd][rbh][rbw] = SIMD_FMADD(
                                    vwt,
                                    SIMD_LOAD(
                                        i + (kd + rbd) * SIMD_WIDTH * 12 * 12 +
                                        (kh + rbh) * SIMD_WIDTH * 12 +
                                        (kw + rbw) * SIMD_WIDTH +
                                        f * 12 * 12 * 1 * SIMD_WIDTH),
                                    vout[rbd][rbh][rbw]);
                            }
                }
...
#pragma unroll(RBW)
            for (int rbw = 0; rbw < RBW; ++rbw)
            {
                SIMD_STORE(o + rbw * SIMD_WIDTH + rbd * SIMD_WIDTH * 100 +
                               rbh * SIMD_WIDTH * 10,
                           vout[rbd][rbh][rbw]);
            }
}
...
int main()
{
    ...
    double gflops =
        static_cast<double>(SIMD_WIDTH * 2 * RBZ * 3 * 3 * SIMD_WIDTH) /
        1000000000;
